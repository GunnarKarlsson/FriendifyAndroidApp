package services
{
	import VOs.ActionsVO;
	import VOs.AlbumVO;
	import VOs.CommentVO;
	import VOs.EventVO;
	import VOs.FriendVO;
	import VOs.GroupVO;
	import VOs.InvitedPersonVO;
	import VOs.NewsFeedVO;
	import VOs.NotificationVO;
	import VOs.PersonVO;
	import VOs.PhotoVO;
	import VOs.PlaceVO;
	...
	public class FBService extends EventDispatcher
	{
		...
		private var _appID:String="310244579014672"; //friendify girly dots
		...
		*/
		public function getGeolocation(updateInterval:int):void
		{
			...
			if (Geolocation.isSupported)
			{
				...
				//geoLocation.setRequestedUpdateInterval(updateInterval);
		...
		*/
		...
		public function getNotifications():void
		{
			...
			//FacebookMobile.api('/me/notifications', handleNotifications, params);
		...
		{
			trace(FacebookMobile.getSession().uid);
			//include read for testing
			//var params:Object=new Object();
			//params["include_read"]=true;
			//FacebookMobile.api('/me/notifications', handleNotificationsPoll, params);
		...
		*/
		...
		public function getNextCommentsPage(id:String):void
		{
			...
			else
			{
				...
				FacebookMobile.api(id + "/comments", handleCommentsResponse, params);
					//FacebookMobile.api('me/home', handleCommentsResponse, params);
			}
		}
		...
		*/
			...
			if (success)
			{
				...
				for (var i:uint=0; i < response.length; i++)
				{
					if (response[i].attachment && response[i].attachment.media && response[i].attachment.media[0] && response[i].attachment.media[0].type == "photo")
					{
						...
						trace("fbs: pid: " + response[i].attachment.media[0].photo.pid);
		...
		private function handleGetEventInvitedList(success:Object, fail:Object):void
		{
			...
			if (success)
			{
				...
				for (var i:uint=0; i < response.length; i++)
				{
					...
					trace("response[i].rsvp_status: "+response[i].rsvp_status);
					...
					for (var i:uint=0; i < response.length; i++)
					{
						...
						if (response[i].link)
						{
							...
							if (indexOfQuestionMark > -1)
							{
								...
								//build params object
								var params:Object=new Object();
								if (params.hasOwnProperty("fbid"))
								{

								}

								if (params.hasOwnProperty("story_fbid"))
								{
		...
		private function handleLogout(response:Object):void
		{
			...
			//else
			//{
			//dispatchEvent(new Event(FBService.LOGOUT_ERROR));
			//}
		}

		private function handleGetNearbyLocationsRequest(s:Object, fail:Object):void
		{
			trace("handleGetCloseLocations");
			...
			if (s)
			{
				for (var i:int=0; i < s.length; i++)
				{
					...
					if (s[i].location)
					{
						...
						trace("FBS vo.latitude" + vo.latitude);
						vo.longitude=s[i].location.longitude;
						trace("FBS vo.longitude" + vo.latitude);
						if (s[i].name)
							vo.name=s[i].name;
						trace("FBS vo.name: " + vo.name);
					}
		...
		{
			trace("FBService loginHandler");
			if (success)
			{

				trace("FBS success login");
		...
		private function handleGetPhotoAlbumsResponse(albums:Object, fail:Object):void
		{
			...
			trace("fbs handleGetPhotoAlbumsResponse");
			photoAlbums=[];
			...
			if (albums != null)
			{
				...
				for (var i:int=0; i < a.length; i++)
				{
					...
					photoAlbums.push(vo);
					trace("name: " + vo.name);
					trace("photoAlbums.length :"+photoAlbums.length);
				}
		...
		/*
		* photos[i].images[?] - standard portrait dimensions
		[0]: h 540 w 720
		[1] h 135 w 180
		[2] h 97 w 130
		[3] h 56 w 75
