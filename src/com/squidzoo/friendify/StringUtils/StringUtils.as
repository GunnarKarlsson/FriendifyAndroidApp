		...
		* }
		...
		public static function base64Encode(p_string:String):String
		{
			...
			while (i < len)
			{
				...
				if (i == len)
				{
					out+=BASE64.charAt(c1 >> 2) + BASE64.charAt((c1 & 0x3) << 4) + '==';
					break;
				}
				var c2:Number=p_string.charCodeAt(i++);
				if (i == len)
				{
					out+=BASE64.charAt(c1 >> 2) + BASE64.charAt(((c1 & 0x3) << 4) | ((c2 & 0xF0) >> 4)) + '=';
					break;
				}
				var c3:Number=p_string.charCodeAt(i++);
				out+=BASE64.charAt(c1 >> 2) + BASE64.charAt(((c1 & 0x3) << 4) | ((c2 & 0xF0) >> 4)) + BASE64.charAt(((c2 & 0xF) << 2) | ((c3 & 0xC0) >> 6)) + BASE64.charAt(c3 & 0x3F);
			}
		...
		public static function editDistance(p_source:String, p_target:String):uint
		{
			...
			for (var b:uint=0; b <= n; b++)
			{
				d[b][0]=b;
			}
			for (var c:uint=0; c <= m; c++)
			{
				d[0][c]=c;
			}
