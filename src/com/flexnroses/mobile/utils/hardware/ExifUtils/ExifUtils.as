package com.flexnroses.mobile.utils.hardware.ExifUtils
{
	...
	public class ExifUtils
	{
		...
		{
			EventCentral.getInstance().dispatchEvent(new DebugEvent(DebugEvent.DEBUG_MESSAGE,"ExifUtils 1"));
		...
		public static function getEyeOrientedBitmap( bitmap:Bitmap, set:IFDSet ):Bitmap
		{
			...
			{
				bitmapData = new BitmapData( bitmap.width, bitmap.height, true,0x00000000 );//added 0x00000000 to make bg transparent
			}
			else
			{
				bitmapData = new BitmapData( bitmap.height, bitmap.width, true,0x00000000 );//added 0x00000000 to make bg transparent
			}
