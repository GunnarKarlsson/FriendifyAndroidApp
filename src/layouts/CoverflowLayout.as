package layouts {
	...
	/*
	* source: com.rialvalue.layouts.CoverflowLayout
	*/
    ...
    public class CoverflowLayout extends LayoutBase {
		...
		//public function getMidElementPosition():Matrix3D{
			//return _dataObject.midElement as Matrix3D;
		//}
        ...
        private function positionCentralElement(element:ILayoutElement, width:Number, height:Number):Matrix3D {
            element.setLayoutBoundsSize(NaN, NaN, false);//NaN,NaN,false
        ...
        override public function updateDisplayList(width:Number, height:Number):void {
            ...
            if (numElements > 0) {
				...
				EventCentral.getInstance().dispatchEvent(new CustomEvent(CustomEvent.MATRIX));
                ...
                for (j = 0, i = midElement + 1; i < numElements; i++, j++) {
                    matrix = positionLateralElement(target.getVirtualElementAt(i), j, RIGHT_SIDE);//matrix = positionLateralElement(target.getVirtualElementAt(i), j, RIGHT_SIDE);
                    finalMatrixs[i] = matrix;
                }
        ...
        private function animationTickHandler(event:TimerEvent):void {
			//trace("cfl animationTickHandleer");
			if(target){
				var numElements:int = target.numElements;
				...
				for (var i:int = 0; i < numElements; i++) {
					...
					//trace( "finalMatrix: "+finalMatrix.position.x, finalMatrix.position.y);
					...
					//trace ("initialMatrix: "+initialMatrix.position.x,initialMatrix.position.y);
        ...
        private function animationTimerCompleteHandler(event:TimerEvent):void {
			//trace("cfl animationTimercompleteHandler");
            finalMatrixs = null;
        }
    }
}